...
export class DeleteIngredientUseCase {
  ...
  async execute({ ingredientId }: DeleteIngredientUseCaseRequest): Promise<DeleteIngredientUseCaseResponse> {
    const ingredientOnDataBase = await this.ingredientsRepository.findById(ingredientId);
