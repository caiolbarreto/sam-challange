...
export class UpdateIngredientUseCase {
  ...
  async execute(ingredient: UpdateIngredientUseCaseRequest): Promise<UpdateIngredientUseCaseResponse> {
    const ingredientOnDataBase = await this.ingredientsRepository.findById(ingredient.ingredientId);

    if (!ingredientOnDataBase) {
      return left(new NotFound('Ingredient not found'));
    }

    await this.ingredientsRepository.update(ingredient.ingredientId, ingredient.content);
